  ...
  <div id="tagManagePage">
    ...
    <div style="display: flex; align-items: center;">
      <a-form layout="inline" :model="searchParams" @finish="doSearch">
        <a-form-item label="标签ID">
          <a-input v-model:value="searchParams.id" placeholder="输入标签ID" allow-clear />
        </a-form-item>
        <a-form-item label="标签名">
          <a-input v-model:value="searchParams.name" placeholder="输入标签名" allow-clear />
        </a-form-item>
        <a-form-item>
    ...
    >
      <a-form :model="addFormData" @finish="handleAddSubmit">
        <a-form-item label="标签名" name="name" :rules="[{ required: true, message: '请输入标签名' }]">
...
const toggleEdit = (id: string) => {
  if (editing.value[id]) {
    ...
    if (record) {
      updateTagUsingPost(record).then(res => {
        if (res.data.code === 0) {
...
const handleAddSubmit = async () => {
  try {
    const res = await addTagUsingPost(addFormData);
    if (res.data.code === 0) {
